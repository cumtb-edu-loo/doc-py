...
## 移除¶
  ...
  * `sys.getcounts()` 函数，`-X showalloccount` 命令行选项以及 C 结构体 [`PyConfig`](../c-api/init_config.md#c.PyConfig "PyConfig") 的 `show_alloc_count` 字段已被移除。 它们需要使用定义了 `COUNT_ALLOCS` 宏的特殊 Python 编译版本。 （由 Victor Stinner 在 [bpo-39489](https://bugs.python.org/issue?@action=redirect&bpo=39489) 中贡献。）
...
### Python API 的变化¶
  ...
  * 更新了 [`__future__`](../library/__future__.md#module-__future__ "__future__: Future statement definitions") 模块中未来特性旗标的常量值以防止与编译器旗标相冲突。 在之前版本中 `PyCF_ALLOW_TOP_LEVEL_AWAIT` 会与 `CO_FUTURE_DIVISION` 发生冲突。 （由 Batuhan Taskaya 在 [bpo-39562](https://bugs.python.org/issue?@action=redirect&bpo=39562) 中贡献。）
...
没有模块状态的扩展模块 (`m_size <= 0`) 不会受到影响。
  ...
  * 多个宏和函数的实现细节现在已被隐藏:
    ...
    * 现在 [`PyIndex_Check()`](../c-api/number.md#c.PyIndex_Check "PyIndex_Check") 总是被声明为不透明函数以隐藏实现细节；`PyIndex_Check()` 宏已被移除。 该宏会直接访问 [`PyTypeObject.tp_as_number`](../c-api/typeobj.md#c.PyTypeObject.tp_as_number "PyTypeObject.tp_as_number") 成员。
...
（由 Victor Stinner 在 [bpo-38644](https://bugs.python.org/issue?@action=redirect&bpo=38644) 和 [bpo-39542](https://bugs.python.org/issue?@action=redirect&bpo=39542) 中贡献。）
  ...
  * 移除了 `Py_UNICODE_MATCH`。 它已被 [**PEP 393**](https://peps.python.org/pep-0393/) 所弃用，并自 Python 3.3 起不再可用。 可以改用 [`PyUnicode_Tailmatch()`](../c-api/unicode.md#c.PyUnicode_Tailmatch "PyUnicode_Tailmatch") 函数。 （由 Inada Naoki 在 [bpo-36346](https://bugs.python.org/issue?@action=redirect&bpo=36346) 中贡献。）
...
[`typing.Literal`](../library/typing.md#typing.Literal "typing.Literal") 的行为被改为遵循 [**PEP 586**](https://peps.python.org/pep-0586/) 并匹配该 PEP 所描述的静态类型检查器的行为。
  ...
  4. 现在如果有任何一个参数不为 [hashable](../glossary.md#term-hashable)，`Literal` 对象将在相等性比较期间引发 [`TypeError`](../library/exceptions.md#TypeError "TypeError")。 请注意使用可变参数声明 `Literal` 将不会抛出异常:
...
现在 [`collections.abc.Callable`](../library/collections.abc.md#collections.abc.Callable "collections.abc.Callable") 泛型会将类型形参展平，类似于 [`typing.Callable`](../library/typing.md#typing.Callable "typing.Callable") 当前所做的那样。 这意味着 `collections.abc.Callable[[int, str], str]` 的 `__args__` 将为 `(int, str, str)`；之前则为 `([int, str], str)`。 为了允许这个改变，[`types.GenericAlias`](../library/types.md#types.GenericAlias "types.GenericAlias") 现在可以被子类化，并且在抽取 [`collections.abc.Callable`](../library/collections.abc.md#collections.abc.Callable "collections.abc.Callable") 类型时将返回一个子类。 通过 [`typing.get_args()`](../library/typing.md#typing.get_args "typing.get_args") 或 `__args__` 访问参数的代码需要考虑到这个改变。 对于无效的 [`collections.abc.Callable`](../library/collections.abc.md#collections.abc.Callable "collections.abc.Callable") 参数化形式可能会发出 [`DeprecationWarning`](../library/exceptions.md#DeprecationWarning "DeprecationWarning")，这在 Python 3.9.1 中可能会静默地传递。 这个 [`DeprecationWarning`](../library/exceptions.md#DeprecationWarning "DeprecationWarning") 将在 Python 3.10 中变为 [`TypeError`](../library/exceptions.md#TypeError "TypeError")。 （由 Ken Jin 在 [bpo-42195](https://bugs.python.org/issue?@action=redirect&bpo=42195) 中贡献。）

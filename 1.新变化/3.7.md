...
当 _name_ 未找到并且 _obj_ 未重载 [`object.__getattr__()`](3.%20数据模型.md#object.__getattr__ "object.__getattr__") 或 [`object.__getattribute__()`](3.%20数据模型.md#object.__getattribute__ "object.__getattribute__") 时 [`hasattr()`](functions.md#hasattr "hasattr") 和 [`getattr()`](functions.md#getattr "getattr") 现在会比原来快大约 4 倍。 （由 INADA Naoki 在 [bpo-32544](https://bugs.python.org/issue?@action=redirect&bpo=32544) 中贡献。）
...
从 [`object.__complex__()`](3.%20数据模型.md#object.__complex__ "object.__complex__") 返回一个 [`complex`](functions.md#complex "complex") 的子类的行为已弃用并将在未来的 Python 版本中引发错误。 这使得 `__complex__()` 的行为与 [`object.__int__()`](3.%20数据模型.md#object.__int__ "object.__int__") 和 [`object.__float__()`](3.%20数据模型.md#object.__float__ "object.__float__") 保持一致。 （由 Serhiy Storchaka 在 [bpo-28894](https://bugs.python.org/issue?@action=redirect&bpo=28894) 中贡献。）
...
### Python 行为的更改¶
  ...
  * [`async`](8.%20复合语句.md#async) 和 [`await`](6.%20表达式.md#await) 现在是保留关键字。 使用了这些名称作为标识符的代码现在将引发 [`SyntaxError`](3.标准库/exceptions.md#SyntaxError "SyntaxError")。 （由 Jelle Zijlstra 在 [bpo-30406](https://bugs.python.org/issue?@action=redirect&bpo=30406) 中贡献。）
  ...
  * [`object.__aiter__()`](3.%20数据模型.md#object.__aiter__ "object.__aiter__") 方法不再能被声明为异步的。 （由 Yury Selivanov 在 [bpo-31709](https://bugs.python.org/issue?@action=redirect&bpo=31709) 中贡献。）
...
现在 Python 3.7 会正确地引发 [`SyntaxError`](3.标准库/exceptions.md#SyntaxError "SyntaxError")，因为生成器表达式总是必须直接包含于一对括号之内， 且前后都不能有逗号，仅在调用时可以忽略重复的括号。 （由 Serhiy Storchaka 在 [bpo-32012](https://bugs.python.org/issue?@action=redirect&bpo=32012) 和 [bpo-32023](https://bugs.python.org/issue?@action=redirect&bpo=32023) 中贡献。）
  ...
  * 现在当使用 [`-m`](1.%20命令行与环境.md#cmdoption-m) 开关时，会将初始工作目录添加到 [`sys.path`](3.标准库/sys.md#sys.path "sys.path")，而不再是一个空字符串（即在每次导入时动态地指明当前工作目录）。 任何会检测该空字符串，或是以其他方式依赖之前行为的的程序将需要进行相应的更新（例如改为还要检测 `os.getcwd()` 或 `os.path.dirname(__main__.__file__)`，具体做法首先要取决于为何要对代码执行空字符串检测）。
...
（由 Serhiy Storchaka 在 [bpo-25054](https://bugs.python.org/issue?@action=redirect&bpo=25054) 和 [bpo-32308](https://bugs.python.org/issue?@action=redirect&bpo=32308) 中贡献。）
  ...
  * 现在可以通过将 `required=True` 传给 [`ArgumentParser.add_subparsers()`](argparse.md#argparse.ArgumentParser.add_subparsers "argparse.ArgumentParser.add_subparsers") 使得 [`argparse`](argparse.md#module-argparse "argparse: Command-line option and argument parsing library.") 子解析器成为必需的。 （由 Anthony Sottile 在 [bpo-26510](https://bugs.python.org/issue?@action=redirect&bpo=26510) 中贡献。）
...
用于覆盖 [`sys.path`](3.标准库/sys.md#sys.path "sys.path") 的文件现在被命名为 `<python-executable>._pth` 而不是 `'sys.path'`。 请参阅 [查找模块](4.%20在Windows上使用%20Python.md#windows-finding-modules) 了解更多信息。 （由 Steve Dower 在 [bpo-28137](https://bugs.python.org/issue?@action=redirect&bpo=28137) 中贡献。）

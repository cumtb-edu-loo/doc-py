...
[`asyncio`](asyncio.md#module-asyncio "asyncio: Asynchronous I/O.") 模块支持 非阻塞 SSL 套接字 并提供了更高层级的 API。 它会使用 [`selectors`](selectors.md#module-selectors "selectors: High-level I/O multiplexing.") 模块来轮询事件并处理 `SSLWantWriteError`, `SSLWantReadError` 和 [`BlockingIOError`](exceptions.md#BlockingIOError "BlockingIOError") 等异常。 它还会异步地执行 SSL 握手。

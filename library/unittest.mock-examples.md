...
通过提供这些附带影响函数，`mock` 的行为将类似于普通字典但又会记录所有访问。 如果你尝试访问一个不存在的键它甚至会引发 [`KeyError`](exceptions.md#KeyError "KeyError")。
...
你可以看到 `import fooble` 成功执行，而当退出时 [`sys.modules`](sys.md#sys.modules "sys.modules") 中将不再有 'fooble'。

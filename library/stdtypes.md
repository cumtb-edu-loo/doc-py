...
字节串（任何遵循 [缓冲区协议](../c-api/buffer.md#bufferobjects) 或是具有 `__bytes__()` 的对象）。
...
`memoryview` 对象允许 Python 代码访问一个对象的内部数据，只要该对象支持 [缓冲区协议](../c-api/buffer.md#bufferobjects) 而无需进行拷贝。
...
Python 的 [`with`](../reference/compound_stmts.md#with) 语句支持通过上下文管理器所定义的运行时上下文这一概念。 此对象的实现使用了一对专门方法，允许用户自定义类来定义运行时上下文，在语句体被执行前进入该上下文，并在语句执行完毕时退出该上下文：
...
调用泛型容器的 [`__getitem__()`](../reference/datamodel.md#object.__getitem__ "object.__getitem__") 方法将引发异常以防出现 `dict[str][str]` 之类的错误:
...
模块唯一的特殊操作是属性访问: `m.name`，这里 _m_ 为一个模块而 _name_ 访问定义在 _m_ 的符号表中的一个名称。 模块属性可以被赋值。 （请注意 [`import`](../reference/simple_stmts.md#import) 语句严格来说也是对模块对象的一种操作；`import foo` 不要求存在一个名为 _foo_ 的模块对象，而是要求存在一个对于名为 _foo_ 的模块的 (永久性) _定义_ 。）
...
关于这些类型请参阅 [对象、值与类型](../reference/datamodel.md#objects) 和 [类定义](../reference/compound_stmts.md#class)。
...
更多信息请参阅 [函数定义](../reference/compound_stmts.md#function)。
...
与函数对象类似，绑定方法对象也支持获取任意属性。 但是，由于方法属性实际上保存于下层的函数对象中 (`meth.__func__`)，因此不允许设置绑定方法的方法属性。 尝试设置方法的属性将会导致引发 [`AttributeError`](exceptions.md#AttributeError "AttributeError")。 想要设置方法属性，你必须在下层的函数对象中显式地对其进行设置:
...
更多信息请参阅 [标准类型层级结构](../reference/datamodel.md#types)。

### 代码对象¶

代码对象被具体实现用来表示“伪编译”的可执行 Python 代码，例如一个函数体。 它们不同于函数对象，因为它们不包含对其全局执行环境的引用。 代码对象由内置的 [`compile()`](functions.md#compile "compile") 函数返回，并可通过从函数对象的 `__code__` 属性从中提取。 另请参阅 [`code`](code.md#module-code "code: Facilities to implement read-eval-print loops.") 模块。

访问 `__code__` 会触发 [审计事件](sys.md#auditing) `object.__getattr__`，参数为 `obj` 和 `"__code__"`。
...
此对象常被用于切片 (参见 [切片](../reference/expressions.md#slicings))。 它不支持任何特殊的操作。 省略符对象只有一种值 [`Ellipsis`](constants.md#Ellipsis "Ellipsis") (这是个内置名称)。 `type(Ellipsis)()` 会生成 [`Ellipsis`](constants.md#Ellipsis "Ellipsis") 单例。

该对象的写法为 `Ellipsis` 或 `...`。

### 未实现对象¶

此对象会被作为比较和二元运算被应用于它们所不支持的类型时的返回值。 请参阅 [比较运算](../reference/expressions.md#comparisons) 了解更多信息。 未实现对象只有一种值 `NotImplemented`。 `type(NotImplemented)()` 会生成这个单例。

该对象的写法为 `NotImplemented`。

### 内部对象¶

有关此对象的信息请参阅 [标准类型层级结构](../reference/datamodel.md#types)。 其中描述了栈帧对象、回溯对象以及切片对象等等。
...
当一个操作会超出限制时，将引发 [`ValueError`](exceptions.md#ValueError "ValueError"):
...
默认限制为 4300 位即 [`sys.int_info.default_max_str_digits`](sys.md#sys.int_info "sys.int_info") 的值。 最低限制可被配置为 640 位即 [`sys.int_info.str_digits_check_threshold`](sys.md#sys.int_info "sys.int_info")。
...
在 Python 启动之前你可以使用环境变量或解释器命令行旗标来配置限制值:
  ...
  * [`PYTHONINTMAXSTRDIGITS`](../using/cmdline.md#envvar-PYTHONINTMAXSTRDIGITS)，例如 `PYTHONINTMAXSTRDIGITS=640 python3` 是将限制设为 640 而 `PYTHONINTMAXSTRDIGITS=0 python3` 是禁用此限制。

  * [`-X int_max_str_digits`](../using/cmdline.md#cmdoption-X)，例如 `python3 -X int_max_str_digits=640`

  * [`sys.flags.int_max_str_digits`](sys.md#sys.flags.int_max_str_digits "sys.flags.int_max_str_digits") 包含 [`PYTHONINTMAXSTRDIGITS`](../using/cmdline.md#envvar-PYTHONINTMAXSTRDIGITS) 或 [`-X int_max_str_digits`](../using/cmdline.md#cmdoption-X) 的值。 如果环境变量和 `-X` 选项均有设置，则 `-X` 选项优先。 值为 _-1_ 表示两者均未设置，因此会在初始化时使用 [`sys.int_info.default_max_str_digits`](sys.md#sys.int_info.default_max_str_digits "sys.int_info.default_max_str_digits") 的值。

从代码中，你可以检查当前的限制并使用这些 [`sys`](sys.md#module-sys "sys: Access system-specific parameters and functions.") API 来设置新值:

  * [`sys.get_int_max_str_digits()`](sys.md#sys.get_int_max_str_digits "sys.get_int_max_str_digits") 和 [`sys.set_int_max_str_digits()`](sys.md#sys.set_int_max_str_digits "sys.set_int_max_str_digits") 是解释器级限制的读取器和设置器。 子解释器具有它们自己的限制。

有关默认值和最小值的信息可在 [`sys.int_info`](sys.md#sys.int_info "sys.int_info") 中找到:

  * [`sys.int_info.default_max_str_digits`](sys.md#sys.int_info "sys.int_info") 是已编译的默认限制。

  * [`sys.int_info.str_digits_check_threshold`](sys.md#sys.int_info "sys.int_info") 是该限制可接受的最低值（禁用该限制的 0 除外）。
...
默认的 [`sys.int_info.default_max_str_digits`](sys.md#sys.int_info.default_max_str_digits "sys.int_info.default_max_str_digits") 被预期对于大多数应用程序来说都是合理的。 如果你的应用程序需要不同的限制值，请使用不预设 Python 版本的代码从你的主入口点进行设置，因为这些 API 是在 3.12 之前的版本所发布的安全补丁中添加的。
...
有关这些特殊方法的额外信息可参看 Python 参考指南 ([基本定制](../reference/datamodel.md#customization))。

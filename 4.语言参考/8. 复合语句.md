...
它通过对表达式逐个求值直至找到一个真值（请参阅 [布尔运算](6.%20表达式.md#booleans) 了解真值与假值的定义）在子句体中选择唯一匹配的一个；然后执行该子句体（而且 `if` 语句的其他部分不会被执行或求值）。 如果所有表达式均为假值，则如果 `else` 子句体如果存在就会被执行。
...
第一个子句体中的 [`break`](7.%20简单语句.md#break) 语句在执行时将终止循环且不执行 `else` 子句体。 第一个子句体中的 [`continue`](7.%20简单语句.md#continue) 语句在执行时将跳过子句体中的剩余部分并返回检验表达式。
...
`starred_list` 表达式会被求值一次；它应当产生一个 [iterable](../glossary.md#term-iterable) 对象。 将针对该可迭代对象创建一个 [iterator](../glossary.md#term-iterator)。 随后该迭代器所提供的第一个条目将使用标准的赋值规则被赋值给目标列表 (参见 [赋值语句](7.%20简单语句.md#assignment))，而代码块将被执行。 此过程将针对该迭代器所提供每个条目重复进行。 当迭代器被耗尽时，如果存在 `else` 子句中的代码块，则它将被执行，并终结循环。

第一个子句体中的 [`break`](7.%20简单语句.md#break) 语句在执行时将终止循环且不执行 `else` 子句体。 第一个子句体中的 [`continue`](7.%20简单语句.md#continue) 语句在执行时将跳过子句体中的剩余部分并转往下一项继续执行，或者在没有下一项时转往 `else` 子句执行。
...
有关异常的更多信息可以在 [异常](4.%20执行模型.md#exceptions) 一节找到，有关使用 [`raise`](7.%20简单语句.md#raise) 语句生成异常的信息可以在 [raise 语句](7.%20简单语句.md#raise) 一节找到。
...
一个 `except*` 子句必须有一个匹配的类型，并且此类型不能是 [`BaseExceptionGroup`](3.标准库/exceptions.md#BaseExceptionGroup "BaseExceptionGroup") 的子类。 不可以将 `except` 和 `except*` 在同一个 `try` 中混用。 [`break`](7.%20简单语句.md#break), [`continue`](7.%20简单语句.md#continue) 和 [`return`](7.%20简单语句.md#return) 不可出现在 `except*` 子句中。

### 8.4.3. `else` 子句¶

如果控制流离开 `try` 子句体时没有引发异常，并且没有执行 [`return`](7.%20简单语句.md#return), [`continue`](7.%20简单语句.md#continue) 或 [`break`](7.%20简单语句.md#break) 语句，可选的 `else` 子句将被执行。 `else` 语句中的异常不会由之前的 `except` 子句处理。

### 8.4.4. `finally` 子句¶

如果存在 `finally`，它将指定一个‘清理’处理句柄。 `try` 子句会被执行，包括任何 `except` 和 `else` 子句。 如果在这些子句中发生任何未处理的异常，该异常会被临时保存。 `finally` 子句将被执行。 如果存在被保存的异常，它会在 `finally` 子句的末尾被重新引发。 如果 `finally` 子句引发了另一个异常，被保存的异常会被设为新异常的上下文。 如果 `finally` 子句执行了 [`return`](7.%20简单语句.md#return), [`break`](7.%20简单语句.md#break) 或 [`continue`](7.%20简单语句.md#continue) 语句，则被保存的异常会被丢弃:
...
当 [`return`](7.%20简单语句.md#return), [`break`](7.%20简单语句.md#break) 或 [`continue`](7.%20简单语句.md#continue) 语句在一个 `try`...`finally` 语句的 `try` 子句的代码块中被执行时，`finally` 子句也会在‘离开时’被执行。

函数的返回值是由最后被执行的 [`return`](7.%20简单语句.md#return) 语句来决定的。 由于 `finally` 子句总是会被执行，因此在 `finally` 子句中被执行的 `return` 语句将总是最后被执行的:
...
在 3.8 版本发生变更: 在 Python 3.8 之前，[`continue`](7.%20简单语句.md#continue) 语句不允许在 `finally` 子句中使用，这是因为具体实现中存在一个问题。

## 8.5. `with` 语句¶

`with` 语句用于包装带有使用上下文管理器 (参见 [with 语句上下文管理器](3.%20数据模型.md#context-managers) 一节) 定义的方法的代码块的执行。 这允许对普通的 `try`...`except`...`finally` 使用模式进行封装以方便地重用。
...
本节使用单引号来表示 [软关键字](2.%20词法分析.md#soft-keywords)。
...
关键字 `match` 和 `case` 是 [soft keywords](2.%20词法分析.md#soft-keywords) 。
...
字面值模式对应 Python 中的大多数 [字面值](2.%20词法分析.md#literals)。 语法为:
...
规则 `strings` 和标记 `NUMBER` 是在 [standard Python grammar](10.%20完整的语法规范.md) 中定义的。支持三引号的字符串。不支持原始字符串和字节字符串。也不支持 [格式字符串字面值](2.%20词法分析.md#f-strings) 。

`signed_number '+' NUMBER` 和 `signed_number '-' NUMBER` 形式是用于表示 [复数](2.%20词法分析.md#imaginary)；它们要求左边是一个实数而右边是一个虚数。 例如 `3 + 4j`。

简而言之， `LITERAL` 只会在 `<subject> == LITERAL` 时匹配成功。对于单例 `None` 、 `True` 和 `False` ，会使用 [`is`](6.%20表达式.md#is) 运算符。
...
捕获模式总是能匹配成功。绑定遵循 [**PEP 572**](https://peps.python.org/pep-0572/) 中赋值表达式运算符设立的作用域规则；名字在最接近的包含函数作用域内成为一个局部变量，除非有适用的 [`global`](7.%20简单语句.md#global) 或 [`nonlocal`](7.%20简单语句.md#nonlocal) 语句。
...
在且仅在任何模式中 `_` 是一个 [软关键字](2.%20词法分析.md#soft-keywords)。 通常情况下它是一个标识符，即使是在 `match` 的目标表达式、`guard` 和 `case` 代码块中也是如此。
...
模式中带点的名称会使用标准的 Python [名称解析规则](4.%20执行模型.md#resolve-names) 来查找。 如果找到的值与目标值比较结果相等则模式匹配成功（使用 `==` 相等运算符）。
...
如果存在位置模式，在匹配前会用类 `name_or_attr` 的 [`__match_args__`](3.%20数据模型.md#object.__match_args__ "object.__match_args__") 属性将其转换为关键词模式。
...
> 参见
>
> [定制类模式匹配中的位置参数](3.%20数据模型.md#class-pattern-matching)
...
在 3.9 版本发生变更: 函数可使用任何有效的 [`assignment_expression`](6.%20表达式.md#grammar-token-python-grammar-assignment_expression) 来装饰。 在之前版本中，此语法则更为受限，详情参见 [**PEP 614**](https://peps.python.org/pep-0614/)。
...
函数调用的语义在 [调用](6.%20表达式.md#calls) 一节中有更详细的描述。 函数调用总是会给形参列表中列出的所有形参赋值，或是用位置参数，或是用关键字参数，或是用默认值。 如果存在 "`*identifier`" 这样的形式，它会被初始化为一个元组来接收任何额外的位置参数，默认为一个空元组。 如果存在 "`**identifier`" 这样的形式，它会被初始化为一个新的有序映射来接收任何额外的关键字参数，默认为一个相同类型的空映射。 在 "`*`" 或 "`*identifier`" 之后的形参都是仅限关键字形参因而只能通过关键字参数传入。 在 "`/`" 之前的形参都是仅限位置形参因而只能通过位置参数传入。
...
创建匿名函数（未绑定到一个名称的函数）以便立即在表达式中使用也是可能的。 这需要使用 lambda 表达式，具体描述见 [lambda 表达式](6.%20表达式.md#lambda) 一节。 请注意 lambda 只是简单函数定义的一种简化写法；在 "`def`" 语句中定义的函数也可以像用 lambda 表达式定义的函数一样被传递或赋值给其他名称。 "`def`" 形式实际上更为强大，因为它允许执行多条语句和使用标注。

**程序员注意事项:** 函数属于一类对象。 在一个函数内部执行的 "`def`" 语句会定义一个局部函数并可被返回或传递。 在嵌套函数中使用的自由变量可以访问包含该 def 语句的函数的局部变量。 详情参见 [命名与绑定](4.%20执行模型.md#naming) 一节。
...
类定义是一条可执行语句。 其中继承列表通常给出基类的列表 (进阶用法请参见 [元类](3.%20数据模型.md#metaclasses))，列表中的每一项都应当被求值为一个允许子类的类对象。 没有继承列表的类默认继承自基类 [`object`](functions.md#object "object")；因此，:
...
随后类体将在一个新的执行帧 (参见 [命名与绑定](4.%20执行模型.md#naming)) 中被执行，使用新创建的局部命名空间和原有的全局命名空间。 （通常，类体主要包含函数定义。） 当类体结束执行时，其执行帧将被丢弃而其局部命名空间会被保存。 [5] 一个类对象随后会被创建，其基类使用给定的继承列表，属性字典使用保存的局部命名空间。 类名称将在原有的全局命名空间中绑定到该类对象。

在类体内定义的属性的顺序保存在新类的 `__dict__` 中。 请注意此顺序的可靠性只限于类刚被创建时，并且只适用于使用定义语法所定义的类。

类的创建可使用 [元类](3.%20数据模型.md#metaclasses) 进行重度定制。
...
在 3.9 版本发生变更: 类可使用任何有效的 [`assignment_expression`](6.%20表达式.md#grammar-token-python-grammar-assignment_expression) 来装饰。 在之前版本中，此语法则更为受限，详情参见 [**PEP 614**](https://peps.python.org/pep-0614/)。

可以在类名之后的方括号中列出 类型形参。 这将向静态类型检查器指明该类是泛型类。 在运行时，可以从类的 `__type_params__` 属性中获取类型参数。 请参阅 泛型类 了解详情。

在 3.12 版本发生变更: 类型形参列表是在 Python 3.12 中新增的。

**程序员注意事项:** 在类定义内定义的变量是类属性；它们将被类实例所共享。 实例属性可通过 `self.name = value` 在方法中设定。 类和实例属性均可通过 "`self.name`" 表示法来访问，当通过此方式访问时实例属性会隐藏同名的类属性。 类属性可被用作实例属性的默认值，但在此场景下使用可变值可能导致未预期的结果。 可以使用 [描述器](3.%20数据模型.md#descriptors) 来创建具有不同实现细节的实例变量。
...
Python 协程的执行可以在多个位置上被挂起和恢复 (参见 [coroutine](../glossary.md#term-coroutine))。 [`await`](6.%20表达式.md#await) 表达式，`async for` 以及 `async with` 只能在协程函数体中使用。
...
另请参阅 [`__aiter__()`](3.%20数据模型.md#object.__aiter__ "object.__aiter__") 和 [`__anext__()`](3.%20数据模型.md#object.__anext__ "object.__anext__") 了解详情。
...
另请参阅 [`__aenter__()`](3.%20数据模型.md#object.__aenter__ "object.__aenter__") 和 [`__aexit__()`](3.%20数据模型.md#object.__aexit__ "object.__aexit__") 了解详情。
...
函数 (包括 协程), 类 和 [类型别名](7.%20简单语句.md#type) 可能包含类型形参列表:
...
类型参数是紧接在函数、类或类型别名的名称之后的方括号 (`[]`) 中声明的。 类型参数可在泛型对象的作用域内访问，但不能在其他地方访问。 因此，在声明 `def func[T](): pass` 之后，模块作用域中就不能再使用 `T` 这个名称。 在下文中，将更精确地描述泛型对象的语义。 类型形参的作用域是用一个特殊函数 (从技术上说，是一个 [标注作用域](4.%20执行模型.md#annotation-scopes)) 来模拟的，它封装了泛型对象的创建操作。
...
对于使用类型形参列表语法声明的 `typing.TypeVar`，范围和约束在创建泛型对象时并不会被求值，只有在通过属性 `__bound__` 和 `__constraints__` 显式地访问它时才会被求值。 要做到这一点，需要在单独的 [标注作用域](4.%20执行模型.md#annotation-scopes) 中对范围和约束进行求值。
...
这里 `annotation-def` 指定了一个 [标注作用域](4.%20执行模型.md#annotation-scopes)，它在运行时并不会实际绑定到任何名称。 （另一项自由是在翻译中达成的：该语法没有通过 [`typing`](typing.md#module-typing "typing: Support for type hints \(see :pep:`484`\).") 模块的属性访问，而是直接创建了一个 [`typing.TypeVar`](typing.md#typing.TypeVar "typing.TypeVar") 的实例）。
...
除了 [`TypeVar`](typing.md#typing.TypeVar "typing.TypeVar") 绑定的 [惰性求值](4.%20执行模型.md#lazy-evaluation) 以外，这等同于:
...
这里还是用 `annotation-def` (不是真正的关键字) 指明 [标注作用域](4.%20执行模型.md#annotation-scopes)，而名称 `TYPE_PARAMS_OF_Bag` 在不会运行时实际被绑定。
...
[`type`](7.%20简单语句.md#type) 语句也可被用来创建泛型类型别名:
...
除了会对值执行 [惰性求值](4.%20执行模型.md#lazy-evaluation) 以外，这等同于:
...
这里，`annotation-def` (不是一个真正的关键字) 指明 [标注作用域](4.%20执行模型.md#annotation-scopes)。 像 `TYPE_PARAMS_OF_ListOrSet` 这样的大写名称不会在运行时实际被绑定。

...
在出错时，大多数 `PyLong_As*` API 都会返回 `(return type)-1`，这与数字无法区分开。请采用 [`PyErr_Occurred()`](exceptions.md#c.PyErr_Occurred "PyErr_Occurred") 来加以区分。
...
这个 [`PyTypeObject`](type.md#c.PyTypeObject "PyTypeObject") 的实例表示 Python 的整数类型。与 Python 语言中的 [`int`](../library/functions.md#int "int") 相同。
